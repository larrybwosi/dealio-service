...
export async function GET(request: Request, { params }: { params: Promise<{ organizationId: string, id: string }> }) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ organizationId: string, id: string }> }) {
  const { organizationId, id } = await params;
  await requirePermission({ organizationId }, 'location:manage');
  ...
  try {
    ...
    const usedCapacity = existingLocation.stockBatches.reduce((sum, batch) => sum + batch.currentQuantity, 0);

    const uniqueProductIds = new Set(existingLocation.stockBatches.map(batch => batch.variant.id));
...
export async function DELETE(request: Request, { params }: { params: Promise<{ organizationId: string, id: string }> }) {
  const { organizationId, id } = await params;
  await requirePermission({ organizationId }, 'location:delete');
