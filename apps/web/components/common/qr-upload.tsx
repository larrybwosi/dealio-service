...
export function QRUploadModal({ trigger, onImageUploaded, onClose }: QRUploadModalProps) {
  ...
  // Generate session and setup Ably when modal opens
  useEffect(() => {
    if (open) {
      initializeSession();
    } else {
  ...
  const initializeSession = () => {
    ...
    // Generate QR code
    const uploadUrl = `https://dealioapi.vercel.app/upload/${newSessionId}`;
    QRCode.toDataURL(uploadUrl, { width: 300, margin: 2 }).then(setQrCodeUrl).catch(console.error);

    // Setup Ably connection
    const ablyChannel = ablyRealtime.channels.get(`session-${newSessionId}`);
    ...
    ablyChannel.subscribe((message: any) => {
      switch (message.name) {
        case 'upload-started':
          ...
          break;
        case 'image-uploaded':
          setUploadedImage(message.data.imageUrl);
          setIsUploading(false);
          break;
      }
    });
    ...
    ablyRealtime.connection.on((stateChange: any) => {
