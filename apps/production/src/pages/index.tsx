...
import { Batch, BatchStatus, Recipe, RecipeIngredient } from '@/types/bakery';
...
const DashboardSkeleton = () => (
  <div className="min-h-screen bg-linear-to-br from-orange-50 to-amber-50 dark:from-gray-950 dark:to-gray-900">
...
export default function Bakery() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-linear-to-br from-orange-50 to-amber-50 dark:from-gray-950 dark:to-gray-900 flex items-center justify-center">
  ...
  const totalInventoryValue = stockData.reduce((total: number, item: StockItem) => {
    const ingredient = ingredients.find((i: RecipeIngredient) => i.id === item.id);
                  ...
                  <div className="space-y-3">
                    {batches.slice(0, 4).map((batch: Batch) => (
                      ...
                      >
                        ...
                        <div className="text-right text-xs text-gray-500 dark:text-gray-400 ml-3 shrink-0">
                          <p>{new Date(batch.date).toLocaleDateString()}</p>
                          <p className="truncate max-w-20">{batch.baker || 'Unassigned'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
