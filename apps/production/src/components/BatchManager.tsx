...
import { BatchStatus, FormattedBatch as ApiFormattedBatch, FormattedBatch } from '@/types/bakery';
...
export default function BatchManager() {
  ...
  const ingredientStock =
    ingredients?.reduce(
      (acc, ingredient) => {
        acc[ingredient.id] = ingredient.currentStock || 0;
        return acc;
      },
      {} as { [key: string]: number }
    ) || {};

  // Type guard to check if the data matches our FormattedBatch structure
  const isFormattedBatch = (batch: any): batch is FormattedBatch => {
    return batch && typeof batch === 'object' && 'batchNumber' in batch && 'recipe' in batch;
  };

  const batches: FormattedBatch[] = batchesData?.data?.filter(isFormattedBatch) || [];

  const filteredBatches = batches.filter(batch => {
    const matchesSearch =
      batch.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      batch.recipe.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (batch.baker && batch.baker.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="space-y-4">
          {filteredBatches.map(batch => (
            <Card key={batch.id} className="bg-background shadow-sm hover:shadow-md transition-shadow">
              ...
              <CardContent>
                {/* Simplified Primary Info Grid */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <Label className="text-sm text-gray-500">Quantity</Label>
                    <p className="font-medium">
                      {batch.plannedQuantity} {batch.unit.symbol}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  {batch.duration && (
                    <div>
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="font-medium">{formatDuration(batch.duration)}</span>
                      </div>
                    </div>
                  )}
                  {batch.productionCost !== undefined && (
                    <div>
                      ...
                      <p className="font-medium">{formattedCurrency(batch.productionCost)}</p>
                    </div>
                  )}
                </div>
