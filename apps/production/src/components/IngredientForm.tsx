...
import { CreateRawMaterialInput } from '@/lib/bakery-validation';
import { CategorySelect } from '@/components/common/category-select';
import { useUnits } from '../common/units';
import { AdvancedUnitSelector } from '../common/units/advance-select';
...
export function CreateIngredientDialog({ open, onOpenChange }: CreateIngredientDialogProps) {
  ...
  const { systemUnits, organizationUnits } = useUnits();
  ...
  const onSubmit = async (data: CreateRawMaterialInput & { categoryId: string }) => {
    try {
      const formData = {
        ...data,
        systemUnitId: selectedUnitType === 'system' ? selectedUnitId : undefined,
        orgUnitId: selectedUnitType === 'org' ? selectedUnitId : undefined,
      };
  ...
  const handleUnitChange = (value: string | null, type: 'system' | 'org') => {
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Select either a system unit or organization unit for this ingredient
          </p>
